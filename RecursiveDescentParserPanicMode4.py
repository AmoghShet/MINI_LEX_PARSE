...
class Parser:
    ...
    def consume(self, expected_type):
        ...
        if self.panic_mode:
            ...
            while self.current_token and self.current_token.type not in ('NEWLINE', 'END'):
                ...
                if self.current_token and (self.current_token.type == 'NEWLINE' or self.current_token.type == 'END'):
                    self.panic_mode = False
                    if self.current_token.type == 'NEWLINE':
                        # Skip 'NEWLINE' token and continue
                        self.current_token = next(self.lexer, None)
                    break
        else:
    ...
    def for_loop(self):
        ...
        if self.current_token.type == 'ID':
            ...
            self.consume('TO')  # Add this line to consume the 'TO' token
            end_expression = self.expression()  # Add this line to consume the expression after 'TO'
            end_value = end_expression.value  # Extract the value from the end expression
    ...
    def expression(self):
        ...
        while self.current_token and not self.panic_mode and self.current_token.type in ('+', '-', '*', '/'):
            ...
            if self.current_token and self.current_token.type not in {'ID', 'INT', 'FLOAT', 'STRING_LITERAL'}:
            ...
            left_term = Node('Value', [Token('ID', value=f'{left_term.value} {operator.value} {right_term.value}')])

        return left_term

    def term(self):
        if self.current_token and self.current_token.type in {'ID', 'INT', 'FLOAT', 'STRING_LITERAL'}:
            cur_token = self.current_token
            self.consume(self.current_token.type)
            return Node('Value', [cur_token])  # Return a Node object instead of Token
        else:
            # Skip term parsing until a synchronization point is found
            self.panic_mode = True
            self.skip_until_sync_point()
            return Node('ErrorRecoveryTerm')

    def skip_until_sync_point(self):
        # Skip tokens until a synchronization point (newline or 'END') is found
        while True:
            print(f"Skipping: {self.current_token.type}")
            self.current_token = next(self.lexer, None)
