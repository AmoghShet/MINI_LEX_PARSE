...
class Parser:
    ...
    def var_declaration(self):
        ...
        while self.current_token.type == 'COMMA':
            self.consume('COMMA')
            self.consume('ID')
        return Node('VarDeclaration', [Node(var_type)])
    ...
    def expression(self):
        ...
        while self.current_token.type in ('+', '-', '*', '/'):
            ...
            left_term = Token('ID', f'({left_term.value})') if left_term.type == 'ID' else left_term
            left_term = Token('ID', f'{left_term.value} {operator.value} {right_term.value}')
        self.current_token = left_term
    ...
    def generate_dot(self, node, dot_content):
        ...
        node_id = id(node)
        label = f'{node.type}: {node.value}' if node.value else node.type
...
# Create a new lexer instance for parsing When you iterate through lexer_instance to print the tokens, you exhaust the generator
lexer_instance = lexer(input_code)
